...
public:
	...
	template<size_t size> Vec2(const T(&)[size]); // Set a Vec2 with an array
	explicit Vec2(const T&); // Set a Vec2 with a single scalar : set to Vec2(val,val)
	Vec2(const T&,const T&); // Set a Vec2 with 2 scalars : set to Vec2(x,y)
	Vec2(const Vec2&); // copy constructor
	Vec2(const Vec2&, const Vec2&); // Create a Vec2 with 2 Vec2
	~Vec2(); // Destructor

	Vec2		add(const Vec2&); // Add a Vec2 to the current Vec2
	Vec2		add(const T&, const T&); // Add 2 scalars to the current Vec2 
	Vec2		add(const T&); // Add a single scalar to the current Vec2
	static Vec2 add(const Vec2&, const Vec2&); // Add a Vec2 to another
	static Vec2 add(const Vec2&, const T&,const T&); // Add a Vec2 to 2 scalars
	static Vec2 add(const T&, const T&,const Vec2&); // Add 2 scalars to a Vec2
	static Vec2 add(const T& x1, const T& y1, const T& x2, const T& y2); // Add 2 scalars to 2 scalars

	Vec2		sub(const Vec2&); // Substract a Vec2 to the current Vec2
	Vec2		sub(const T&, const T&); // Substract 2 scalar to the current Vec2 
	Vec2		sub(const T&); // Substract a single scalar to the current Vec2
	static Vec2 sub(const Vec2&, const Vec2&); // Sub a Vec2 to another
	static Vec2 sub(const Vec2&, const T&, const T&); // Sub a Vec2 to 2 scalars
	static Vec2 sub(const T&, const T&, const Vec2&); // Sub 2 scalars to a Vec2
	static Vec2 sub(const T& x1, const T& y1, const T& x2, const T& y2); // Add 2 scalars to 2 scalars

	Vec2		mult(const Vec2&); // Multiplie a Vec2 to the current Vec2
	Vec2		mult(const T&, const T&); // Multiplie 2 scalars to the current Vec2 
	Vec2		mult(const T&); // Multiplie a single scalar to the current Vec2
	static Vec2 mult(const Vec2&, const Vec2&); // Multiplie a Vec2 to another
	static Vec2 mult(const Vec2&, const T&, const T&); // Multiplie a Vec2 to 2 scalars
	static Vec2 mult(const T&, const T&, const Vec2&); // Multiplie 2 scalars to a Vec2
	static Vec2 mult(const T& x1, const T& y1, const T& x2, const T& y2); // Multiplie 2 scalars to 2 scalars

	Vec2		div(const Vec2&); // Divide a Vec2 to the current Vec2
	Vec2		div(const T&, const T&); // Divide 2 scalars to the current Vec2 
	Vec2		div(const T&); // Divide a single scalar to the current Vec2
	static Vec2 div(const Vec2&, const Vec2&); // Divide a Vec2 to another
	static Vec2 div(const Vec2&, const T&, const T&); // Divide a Vec2 to 2 scalars
	static Vec2 div(const T&, const T&, const Vec2&); // Divide 2 scalars to a Vec2
	static Vec2 div(const T& x1, const T& y1, const T& x2, const T& y2); // Divide 2 scalars to 2 scalars
...
inline Vec2<T>::Vec2(const T(&arr)[size])
{
	if (size >= 2) {
		this->x = arr[0];
		this->y = arr[1];
